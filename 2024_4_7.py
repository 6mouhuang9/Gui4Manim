...
def collect_object_params(object_count):
    ...
    for i in range(object_count):
        # 对象的主框架
        object_frame = ttk.LabelFrame(root, text=f"对象 {i+1}")
        ...
        # 添加一个子框架专门用于动画配置
        animations_config_frame = ttk.Frame(object_frame)
        animations_config_frame.pack(fill='x', expand=True)

        # 确认动画配置的操作
        def confirm_animation_config():
            # 清除旧的动画配置
            for widget in animations_config_frame.winfo_children():
                widget.destroy()

            try:
                animation_count = int(animation_count_entry.get())
                create_animation_config(animations_config_frame, animation_count)
            except ValueError:
                print("请输入有效的动画数量")
...
def create_animation_config(frame, animation_count):
    """动态生成动画配置界面"""
