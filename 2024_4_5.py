...
def on_object_type_change(index, object_type_var, params_frame):
    """根据选定的对象类型更新参数输入区域"""
    # 先清除旧的参数输入字段
    ...
    # 创建新的参数输入字段
    for param, info in OBJECTS_PARAMS[selected_object_type].items():
        type_name = info['type']
        ttk.Label(params_frame, text=f"{param} ({type_name}):").pack()
...
def collect_object_params(object_count):
    root = tk.Tk()
    root.title("选择对象类型及其参数")

    for i in range(object_count):
        object_frame = ttk.Frame(root)
        object_frame.pack(padx=10, pady=5, fill='x', expand=True)
        
        # 对象类型选择
        object_type_var = tk.StringVar(value=list(OBJECTS_PARAMS.keys())[0])
        ttk.Label(object_frame, text="对象类型:").pack(side=tk.LEFT)
        object_type_menu = ttk.OptionMenu(object_frame, object_type_var, object_type_var.get(), *OBJECTS_PARAMS.keys())
        object_type_menu.pack(side=tk.LEFT)
        
        # 参数输入区域
        params_frame = ttk.Frame(root)
        params_frame.pack(padx=10, pady=5, fill='x', expand=True)
        
        # 为对象类型变量添加回调函数，以便在变化时更新参数输入区域
        object_type_var.trace_add('write', lambda *args, otv=object_type_var, pf=params_frame: on_object_type_change(i, otv, pf))
        
        # 初始化参数输入区域
        on_object_type_change(i, object_type_var, params_frame)
